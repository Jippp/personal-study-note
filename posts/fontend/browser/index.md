...
所以解释一下上面的TCP过程：
1. 首先客户端处于CLOSED状态，这是需要建立TCP连接了，所以客户端发起了一个**SYN包(表示需要建立连接)**，其中包含了客户端的初始序列号x **seq=x(验证服务端接受数据是否正常)**。客户端发送SYN包之后进入SYN_SENT状态(连接发送状态)。
...
> [!NOTE]
> HTTP2之前一个域名同一时间只能有4-6个请求，这个限制导致了 请求可能需要排队。
