...
- 首次原则：用户首次打开网站或应用时，需要以足够快的速度获取页面。
- 持续性原则：用户在后续是用网站或应用时，能够迅速响应用户操作。
...
比如页面还没有渲染完成，正在执行一段长任务，用户开始交互，浏览器执行完这段长任务到开始响应用户交互，这中间的时间就是FID。一般是长任务的执行时间。
...
也就是说TTI从 FCP的起点，到保持5s静默状态前的最后一个长任务结束时间。
...
该指标是一个比值，有自己的计算公式。
在页面加载过程中，可见元素发生偏移，会通过一个公式来计算出CLS的值。
...
往往开发时能通过浏览器的devtools面板看到这些性能指标，但是在用户的电脑上我们可看到不上，所以需要添加额外的代码来监控性能指标，以达到性能优化的目的。
...
性能监控往往是在生产环境应用的，开发环境中还是需要手动查看性能指标，以便在开发环境的性能优化。
...
从发出请求到接收到服务端响应，先后有DNS解析、TCP连接、https的TSL安全连接、http请求。
...
为什么堆要慢？
- 想要读取对线属性，首先要在栈中取到堆的引用地址，然后去堆中遍历找到该地址对应的内存，再去该内存中取到属性，如果是嵌套的对象，还要继续遍历，直到找到属性。
...
从TCP/IP四层模型的方向来看：
- 应用层主要的优化方向：浏览器的加载和渲染方向的优化，细化就是资源的加载、拆分、打包等。
- 传输层的优化方向：包裹端口号、需要双方IP给下一层，这里的优化主要是TCP协议的优化
- 网络层的优化方向：包裹IP、需要双方MAC给下一层，这里的优化前端方向就更少了
- 链路层的优化方向：包裹MAC，前端基本做不到这里，一般由运维来处理。
