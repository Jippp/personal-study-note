...
- View：视图层，展示数据，并且接收用户的输入以及对数据的修改。
- Controller：中介的作用，根据动作来修改Model和View。包含应用程序的业务逻辑。
...
这个需求的背景是这样的：我是在需求评审完成之后才被拉去参与开发的，所以对需求的了解不是很深；另外该需求是一个迭代需求，所以是存在大量旧代码的。

然后遇到的难点就是：在旧代码的基础上拓展新的功能，因为旧代码是赶工赶出来的，注释少，耦合度多。举个例子一个函数的入参有十几个，而且没有相关的注释，这是非常难维护的。

针对这个问题应对方案就是：主动和之前的开发了解代码逻辑，同时和产品及时沟通确定业务需求。

最后也是成功重构了部分代码，并且完成了新功能的开发工作。

还有一个难点是性能瓶颈问题：这个需求要求用户能通过配置添加树节点，并且两棵树能相互交互增加、删除、易懂、修改等。性能问题主要是接口太慢，因为树节点包含的信息可能很多，并且能批量操作。主要问题就是一期时后端的技术方案是将数据直接存到了mysql里，所以这里就出现了性能问题。

后续的解决方案：就是和上下游，领导、后端、产品、ui等及时开会沟通，让后端通过redis来存储数据并且提供新接口，支持增量加载。进入页面时只返回一级节点数据，需要交互时再通过接口拿到具体数据处理；批量操作时只传递页面展示的必要信息，然后在交互时再去获取具体的节点信息。
